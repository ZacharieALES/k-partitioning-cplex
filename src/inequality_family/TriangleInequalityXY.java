...
public class TriangleInequalityXY extends AbstractInequality<IFEdgeVNodeClusterV>{

	public int s1;
	public int t1, t2;
	
	public TriangleInequalityXY(IFEdgeVNodeClusterV formulation, int s1, int t1, int t2) {
	...
	public AbstractInequality<IFEdgeVNodeClusterV> clone() {
		...
		return new TriangleInequalityXY(formulation, s1, t1, t2);
	}

	@Override
	protected double evaluate(VariableGetter vg) throws IloException  {

		double result =  vg.getValue(formulation.nodeInClusterVar(s1, t1));
		result += vg.getValue(formulation.nodeInClusterVar(s1, t2));
