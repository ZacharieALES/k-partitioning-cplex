 ...
 *
 */
public class BranchCB_Rep_first_descending_test extends BranchCallback{
	
	PartitionWithRepresentative rep;
		
//	private static final Logger logger = Logger.getLogger("");
	
	public BranchCB_Rep_first_descending_test(PartitionWithRepresentative rep){
		this.rep = rep;
	}
...
//				this.makeBranch(vars, bounds, dirs, this.getObjValue());
				...
				/* Branch on x2 = 1 (-> x3 = 0) */
				this.makeBranch(rep.eq(0.0, expr12), this.getObjValue());
...
//						dirs[i-1] = BranchDirection.Down;
						...
						c_range[i-1] = rep.eq(0.0, expr_i_nn);
					}
...
//					dirs[nextNode-1] = BranchDirection.Up;
					c_range[nextNode-1] = rep.eq(1.0, expr);
...
//					dirs[v] = BranchDirection.Up;
					IloLinearNumExpr expr_12 = rep.linearNumExpr();
					expr_12.addTerm(1.0,rep.v_edge[1][0]);
					c_range[v] = rep.eq(1.0, expr_12);
					v++;

//					vars[v] = rep.v_edge[2][0];
//					bounds[v] = 1.0;
//					dirs[v] = BranchDirection.Up;
					IloLinearNumExpr expr_13 = rep.linearNumExpr();
					expr_13.addTerm(1.0,rep.v_edge[2][0]);
					c_range[v] = rep.eq(1.0, expr_13);
					v++;

//					vars[v] = rep.v_edge[2][1];
//					bounds[v] = 1.0;
//					dirs[v] = BranchDirection.Up;
					IloLinearNumExpr expr_23 = rep.linearNumExpr();
					expr_23.addTerm(1.0,rep.v_edge[2][1]);
					c_range[v] = rep.eq(1.0, expr_23);
					v++;
...
//						dirs[nextNode + i - 4] = BranchDirection.Down;
						...
						c_range[nextNode + i - 4] = rep.eq(0.0, expr_i);		
...
//							bounds[v] = 1.0;
//							dirs[v] = BranchDirection.Up;
							IloLinearNumExpr expr_i_j = rep.linearNumExpr();
							expr_i_j.addTerm(1.0,rep.v_edge[i-1][j-1]);
							c_range[v] = rep.eq(1.0, expr_i_j);
...
//					logger.log(Level.FINER, "Branching on x" + nextNode + " = 0");
					...
					this.makeBranch(rep.eq(0.0, expr), this.getObjValue(), (Object)new_ni);
...
//							dirs[i - 4] = BranchDirection.Up;
							IloLinearNumExpr expr_i = rep.linearNumExpr();
							expr_i.addTerm(1.0,rep.v_rep[i - 4]);
							c_range2[i - 4] = rep.eq(1.0, expr_i);
...
//								dirs[v] = BranchDirection.Down;
								...
								c_range2[v] = rep.eq(0.0, expr_i_nn);
								v++;
...
//						dirs[nextNode - 4] = BranchDirection.Down;
						c_range2[nextNode - 4] = rep.eq(0.0, expr);
...
//				this.makeBranch(rep.v_rep[rep.n - 4], 1.0, BranchDirection.Up, this.getObjValue(), (Object)new_ni);
				this.makeBranch(rep.eq(1.0, expr), this.getObjValue(), (Object)new_ni);
...
//				this.makeBranch(rep.v_rep[rep.n - 4], 0.0, BranchDirection.Down, this.getObjValue(), (Object)new_ni1);
				this.makeBranch(rep.eq(0.0, expr), this.getObjValue(), (Object)new_ni1);	
