 ...
 *
 */
public class BranchCBRepFirstDescendingTest extends BranchCallback{
	
	PartitionWithRepresentative rep;
		
//	private static final Logger logger = Logger.getLogger("");
	
	public BranchCBRepFirstDescendingTest(PartitionWithRepresentative rep){
		this.rep = rep;
	}
...
//				logger.log(Level.FINER, "Branching on x3");
				...
				IloLinearNumExpr expr12 = rep.getCplex().linearNumExpr();
				expr12.addTerm(1.0, rep.v_edge[1][0]);
				c_range[0] = rep.getCplex().eq(1.0, expr12);

				IloLinearNumExpr expr13 = rep.getCplex().linearNumExpr();
				expr13.addTerm(1.0, rep.v_edge[2][0]);
				c_range[1] = rep.getCplex().eq(0.0, expr13);

				IloLinearNumExpr expr23 = rep.getCplex().linearNumExpr();
				expr23.addTerm(1.0, rep.v_edge[2][1]);
				c_range[2] = rep.getCplex().eq(0.0, expr23);
...
//				this.makeBranch(vars, bounds, dirs, this.getObjValue());
				...
				/* Branch on x2 = 1 (-> x3 = 0) */
				this.makeBranch(rep.getCplex().eq(0.0, expr12), this.getObjValue());
...
//						dirs[i-1] = BranchDirection.Down;
						...
						c_range[i-1] = rep.getCplex().eq(0.0, expr_i_nn);
					}
...
//					dirs[nextNode-1] = BranchDirection.Up;
					c_range[nextNode-1] = rep.getCplex().eq(1.0, expr);
...
//					dirs[v] = BranchDirection.Up;
					IloLinearNumExpr expr_12 = rep.getCplex().linearNumExpr();
					expr_12.addTerm(1.0,rep.v_edge[1][0]);
					c_range[v] = rep.getCplex().eq(1.0, expr_12);
					v++;

//					vars[v] = rep.v_edge[2][0];
//					bounds[v] = 1.0;
//					dirs[v] = BranchDirection.Up;
					IloLinearNumExpr expr_13 = rep.getCplex().linearNumExpr();
					expr_13.addTerm(1.0,rep.v_edge[2][0]);
					c_range[v] = rep.getCplex().eq(1.0, expr_13);
					v++;

//					vars[v] = rep.v_edge[2][1];
//					bounds[v] = 1.0;
//					dirs[v] = BranchDirection.Up;
					IloLinearNumExpr expr_23 = rep.getCplex().linearNumExpr();
					expr_23.addTerm(1.0,rep.v_edge[2][1]);
					c_range[v] = rep.getCplex().eq(1.0, expr_23);
					v++;
...
//							bounds[v] = 1.0;
//							dirs[v] = BranchDirection.Up;
							IloLinearNumExpr expr_i_j = rep.getCplex().linearNumExpr();
							expr_i_j.addTerm(1.0,rep.v_edge[i-1][j-1]);
							c_range[v] = rep.getCplex().eq(1.0, expr_i_j);
...
//					this.makeBranch(vars, bounds, dirs, this.getObjValue(), (Object)new_ni1);
				...
				int value = (nextNode - 2)  - (rep.KMax() - ni.nbRepTo1);
...
//					logger.log(Level.FINER, "Branching on x" + nextNode + " = 0");
					...
					this.makeBranch(rep.getCplex().eq(0.0, expr), this.getObjValue(), (Object)new_ni);
...
//						dirs[v] = BranchDirection.Down;
						IloLinearNumExpr expr_12 = rep.getCplex().linearNumExpr();
						expr_12.addTerm(1.0,rep.v_edge[1][0]);
						c_range2[v] = rep.getCplex().eq(0.0, expr_12);
						v++;
...
//						dirs[v] = BranchDirection.Down;
						IloLinearNumExpr expr_13 = rep.getCplex().linearNumExpr();
						expr_13.addTerm(1.0,rep.v_edge[2][0]);
						c_range2[v] = rep.getCplex().eq(0.0, expr_13);
						v++;
...
//						dirs[v] = BranchDirection.Down;
						IloLinearNumExpr expr_23 = rep.getCplex().linearNumExpr();
						expr_23.addTerm(1.0,rep.v_edge[2][1]);
						c_range2[v] = rep.getCplex().eq(0.0, expr_23);
						v++;
...
//								dirs[v] = BranchDirection.Down;
								...
								c_range2[v] = rep.getCplex().eq(0.0, expr_i_nn);
								v++;
...
//						dirs[nextNode - 4] = BranchDirection.Down;
						c_range2[nextNode - 4] = rep.getCplex().eq(0.0, expr);
...
//				this.makeBranch(rep.v_rep[rep.n - 4], 1.0, BranchDirection.Up, this.getObjValue(), (Object)new_ni);
				this.makeBranch(rep.getCplex().eq(1.0, expr), this.getObjValue(), (Object)new_ni);
...
//				this.makeBranch(rep.v_rep[rep.n - 4], 0.0, BranchDirection.Down, this.getObjValue(), (Object)new_ni1);
				this.makeBranch(rep.getCplex().eq(0.0, expr), this.getObjValue(), (Object)new_ni1);	
