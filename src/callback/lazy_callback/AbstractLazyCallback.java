...
//			this.add(rep.le(expr, ubound));
			this.add(formulation.getCplex().le(expr, ubound), IloCplex.CutManagement.UseCutFilter);
		} catch (IloException e) {
			e.printStackTrace();
			System.exit(0);
		}
	...
	public void addGe(IloLinearNumExpr expr, double lbound){
		try {
			this.add(formulation.getCplex().ge(expr, lbound), IloCplex.CutManagement.UseCutFilter);
		} catch (IloException e) {
			e.printStackTrace();
		}
	}
