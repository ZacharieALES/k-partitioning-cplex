 ...
 *
 */
public class N_minus_k_fusion implements Abstract_MIPStartGetter{
	
	Solution_Representative s;
	int mergedNodes = 0;
	
	public N_minus_k_fusion(Solution_Representative s){
		this.s = s;
	}
	...
	public MIPStart getMIPStart(){
		...
		while(it.hasNext() && mergedNodes < s.n() - s.K()){
			Edge e = it.next();
