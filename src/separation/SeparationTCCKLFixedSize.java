...
public class SeparationTCCKLFixedSize extends AbstractKLSeparation<IFEdgeV>{
	...
	public SeparationTCCKLFixedSize(IFEdgeV formulation, VariableGetter vg, int iterations, ArrayList<Integer> sizesCycle, boolean stopIteratingAfterCutFound){
	...
	public void updateScore(int positionInC, int previousNode, int newNode, int sizeC) throws IloException{
		...
		for(int i = 0 ; i < formulation.n() ; ++i){
			...
			if(i != newNode && i != previousNode){
				...
				double value = - vg.getValue(formulation.edgeVar(i, newNode)) + vg.getValue(formulation.edgeVar(i, previousNode));
