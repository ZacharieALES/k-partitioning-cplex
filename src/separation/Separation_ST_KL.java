...
/**
 * Use Kernighan-Lin type of algorithm to find violated ST-inequalities violated (with S and T two disjoint subsets such that 0<|S|<|T|):
 * x(S:T) - x(S) - x(T) <= |S|
 * 
 ...
 * 		do a phase:
 * 			Apply transformations until no transformation remain (a transformation can only happened one time).
 * 		until the best inequality found in the phase is better than the best found in the previous phase  
 * 		Add the best inequality found.
 * 
 * @author zach
 *
 */
public class Separation_ST_KL extends Abstract_KL_Separation{
	 ...
	 * 
	 * move[i][j] is equal to worstValue if the transformation have already been done or if the node i started in the set j. 
	 */
	double[][] move;
		
	public Separation_ST_KL(Solution_Representative ucc, int iterations, boolean stopIteratingAfterCutFound) {
		super("ST_KL", ucc, iterations, stopIteratingAfterCutFound);
	}
	
	@Override
	public void setSets(Abstract_Inequality sets) {
		currentSets = ((ST_Inequality)sets).clone();
	}
	...
	public void initializeSets() {
		...
		if(currentSet().S.size() == 0){
			...
			int s = random.nextInt(this.s.n());
			
			if(currentSet().T.size() < this.s.n())
	...
	public void initializeSubSlacks() {
		 ...
		 * 
		 *  Ex: cost of S -> T
		 * 		If s goes from S to T, the slack of the ST-inequality (|S| - x(S,T) + x(S) + x(T)) is modified by 2 (x(s,T) - x(s,S2) + 1
		 * 		since |S2| - x(S2, T+s) + x(T+s) + x(S2) -  = |S| - x(S,T) + x(T) + x(S) -  2 (x(s, S2) - x(s,T)) - 1
		 * 
		 * The cost of the transformations are:
		 * 		1 : S -> T  : 2(x(s,T ) - x(s,S2)) - 1
		 * 		2 : S -> U  :   x(s,T ) - x(s,S2)  - 1
		 * 		3 : T -> S  : 2(x(t,S ) - x(t,T2)) + 1
		 * 		4 : T -> U  :   x(t,S) - x(t,T2)
		 * 		5 : U -> S  :   x(u,S ) - x(u,T )  + 1
		 * 		6 : U -> T  :   x(u,T ) - x(u,S )
		 * 		7 : S <-> T : 2(x(s,T2) + x(t,S2)  - x(t,T2) - x(s,S2))
		 * 		8 : S <-> U :   x(u,S2) + x(s,T )  - x(s,S2) - x(u,T )  
		...
		for(int s = 0 ; s < currentSet().S.size() ; s++){
			...
			for(int s2 = 0 ; s2 < currentSet().S.size() ; ++s2)
				if(s != s2){
					xiS[s_id] += this.s.x(s_id,currentSet().S.get(s2));
				}
			
			for(int t = 0 ; t < currentSet().T.size() ; ++t){
				xiT[s_id] += this.s.x(s_id,currentSet().T.get(t));
			}
		...
		for(int t = 0 ; t < currentSet().T.size() ; t++){
			...
			for(int t2 = 0 ; t2 < currentSet().T.size() ; ++t2)
				if(t!= t2){
					xiT[t_id] += s.x(t_id,currentSet().T.get(t2));
				}
			
			for(int s = 0 ; s < currentSet().S.size() ; ++s){
				xiS[t_id] += this.s.x(t_id, currentSet().S.get(s));
			}
		...
		for(int s = 0 ; s < currentSet().S.size() ; ++s){
			...
			for(int t = 0 ; t < currentSet().T.size() ; ++t)
				leftHandSide += this.s.x(currentSet().S.get(s), currentSet().T.get(t));
			
			for(int s2 = s+1 ; s2 < currentSet().S.size() ; ++s2)
				leftHandSide -= this.s.x(currentSet().S.get(s), currentSet().S.get(s2));
		}
		
		for(int t = 0 ; t < currentSet().T.size() ; ++t)
			for(int t2 = t+1 ; t2 < currentSet().T.size() ; ++t2)
				leftHandSide -= this.s.x(currentSet().T.get(t), currentSet().T.get(t2));
		...
		for(int u = 0 ; u < s.n() ; u++){
			...
			if(!currentSet().inS[u] && !currentSet().inT[u]){
				...
				for(int s = 0 ; s < currentSet().S.size() ; ++s)
					xiS[u] += this.s.x(u, currentSet().S.get(s));
				
				for(int t = 0 ; t < currentSet().T.size() ; ++t)
					xiT[u] += s.x(u, currentSet().T.get(t));
			}
...
//						frac = (leftHandSide - move[i][2] + 1E-6) - (set.S.size() + tempRHSC);
				...
				for(int t = 0 ; t < set.T.size() ; t++){
					...
					/* 7 : S <-> T : -2(x(t,T2) + x(s,S2)) - x(s,T2) - x(t,S2) */
					if(exchange[i][t_id] != worstValue){
						double xst = s.x(i, t_id);
...
//						frac = (leftHandSide - move[i][1] + 1E-6) - (set.S.size());
				...
				for(int s = 0 ; s < set.S.size() ; ++s){
					...
					double xus = this.s.x(i, s_id);

					/* 8 : S <-> U :   - x(u,T ) -  x(s,S2) + x(s,T ) + x(u,S2) */
...
//							frac = (leftHandSide - exchange[t_id][i] + 1E-6) - (set.S.size());
	...
	public ST_Inequality currentSet(){
		return (ST_Inequality)currentSets;
	}
