 ...
 * Let C be an odd cycle of size 2p+1 and Cb its two-chords. The corresponding TCC inequality is:
 * x(C)-x(Cb) <= p
 * 
 * 
 ...
 * 		- Randomly generate C. 
 * 		- Apply transformations until no transformation remain (a transformation can only occur once).
 * 		- Add the best inequality found.  
 * 
 * @author zach
 *
 */
public class Separation_TCC_KL_Fixed_size extends Abstract_KL_Separation{
	 ...
	 * 
	 * exchange[i][j] is equal worstValue if the transformation have already been done or if the nodes i and j are both not in C
	 */
	double[][] exchange;
	...
	public Separation_TCC_KL_Fixed_size(Solution_Representative rep, int iterations, ArrayList<Integer> sizesCycle, boolean stopIteratingAfterCutFound){
		super("TCC_KL", rep, iterations, stopIteratingAfterCutFound);
	...
	public void setSets(Abstract_Inequality sets) {
		...
		currentSets = ((TCC_Inequality)sets).clone();
	...
	public void updateSets(Transformation t) {
		...
		/* If only c2 is in C (it's sure in that case since at least one of the two nodes must be in C */
		else {
	...
	public void initializeSets() {
		...
		currentSets = new TCC_Inequality(s, size);
		TCC_Inequality set = currentSet();
	...
	public void initializeSubSlacks() {
		 ...
		 * 		If c and u are exchanged, the cost of the TCC-inequality x(C)-x(Cb) is modified by - C1 + C2 with
		 * 			- "-C1": cost of adding u in C : - (-x_a,u + x_b,u + x_d,u - x_e,u) = - xiC[u][p_c]
		 * 			- "-C2": cost of removing c from C : -x_a,c + x_b,c + x_d,c - x_e,c = + xiC[c][p_c]
		 * 		
		 */
		xiC = new double[s.n()][size];
	...
	public void computeTransformationSlacks() {
		...
		for(int c = 0 ; c < size ; c++){
			...
			for(int i = 0 ; i < s.n() ; ++i){
				...
				/* If i is not c_id
				 * and if i is not in C or if is in C but the exchange i <-> c is possible (i.e. they are at least at a distance of 3 in the cycle)
				 */
//				if(i != c_id && (inC[i] == -1 || exchange[i][c_id] != worstValue)){
				if(i != c_id && exchange[i][c_id] != worstValue){

						/* Set the cost for setting i in the cycle at position c_pos and for removing c_id from this position */
						...
						if(pos_i != -1){
							...
							/* Add the cost for setting c_id in the cycle at position pos_i and for removing i from this position */
	...
	public void updateScore(int positionInC, int previousNode, int newNode, int sizeC){
		...
		for(int i = 0 ; i < s.n() ; ++i){
			...
			if(i != newNode && i != previousNode){
				...
				double value = - s.x(i, newNode) + s.x(i, previousNode);
...
//		xiC[newNode][(positionInC+2)%sizeC] = worstValue;
		...
		/* previousNode can still not be exchanged with its previous neighbors and now it can't be exchange in position positionInC */
//		xiC[previousNode][positionInC] = worstValue;
	...
	public TCC_Inequality currentSet(){
		return (TCC_Inequality)currentSets;
	}
