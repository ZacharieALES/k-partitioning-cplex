...
//	Double lowerBound = null;
	...
	public Separation_DependentSet_KL(Solution_Representative ucc, int iterations, boolean stopIteratingAfterCutFound) {
		super("DependentSet_KL ", ucc, iterations, stopIteratingAfterCutFound);
	}
	
	@Override
	public void setSets(Abstract_Inequality sets) {
		currentSets = ((DependentSet_Inequality)sets).clone();
		q = currentSet().Z.size() / s.K();
		r = currentSet().Z.size() % s.K();
	}
...
//TODO Verifier qu'on ne descend pas en dessous de K+1 en taille de set
	...
	public void initializeSets() {
		...
		int size = random.nextInt(s.n() - s.K()) + 1 + s.K();
...
//this.displaySet();		
	...
	public void initializeSubSlacks()  throws IloException {
		...
		xiZ = new double[s.n()];
		setWeight = 0.0;
		lower_bound = q/2 * (r + currentSet().Z.size() - s.K());
