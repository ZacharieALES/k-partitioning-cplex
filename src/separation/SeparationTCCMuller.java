 ...
 *
 */
public class SeparationTCCMuller extends AbstractSeparation<IFEdgeV>{
	
	public SeparationTCCMuller(IFEdgeV formulation, VariableGetter vg) {
		super("TCC_Muller", formulation, vg);
	...
	public ArrayList<AbstractInequality<? extends IFormulation>> separate(){
		...
		int n2 = 4*formulation.n()*(formulation.n()-1);
		...
		for(int i = 0 ; i< formulation.n()-1 ; i++)
			for(int j = i+1 ; j < formulation.n() ; j++){
				...
				dist[id_ij+2][id_ij+3] = - vg.getValue(formulation.edgeVar(i,j))
						+ eps;
...
//					dist[id_ij+3][id_ij+2] = dist[id_ij+2][id_ij+3];
				...
				for(int k = 0 ; k < formulation.n() ; k++)
					...
					if(k!=i && k!=j){
						...
						dist[id_ij+1][id_ik+2] = vg.getValue(formulation.edgeVar(j,k)) + 0.5 + eps;
						dist[id_ij+3][id_ik  ] = vg.getValue(formulation.edgeVar(j,k)) + 0.5 + eps;
...
//System.out.println(range + "\n--");
	...
	private int id(int i, int j){
		 ...
		 * -> i * (4n-2i-2) + 4*(j-i-1)
		 */	
		return i*(4*formulation.n()-2*i-2)+4*(j-i-1);
...
//String s = "";	
			...
			expr.addTerm(1.0, formulation.edgeVar(previousNode,currentNode));
			expr.addTerm(-1.0, formulation.edgeVar(previousNode2,currentNode));
