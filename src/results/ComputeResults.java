...
public class ComputeResults {
	...
	public static String[][] meanRatio(double[][][] up, double[][][] down, int signNb, int nm, int nM, int km, int kM){
...
//			System.out.println();
	...
	public static String[][] meanImprovement(double[][][] ref, double[][][] compared, int signNb, int nm, int nM, int km, int kM){
...
//				result += improvement(Math.max(ref[i], comparedTo[i]),Math.max(ref[i], comparedTo[i]));	
				result = -1.0;
...
//				output = nf1.format(result);
	...
	public static String get3DTables(ArrayList<double[][][]> time, ArrayList<double[][][]> branch, ArrayList<double[][][]> cut, ArrayList<double[][][]> bestInt, ArrayList<double[][][]> bestRelax, ArrayList<String> solutionHeaders){
	...
	public static String get3DTables(CP_ResultParamToDisplay param, Result[][][] result, int[] nValue, int[] kValue, String[] solutionHeader){
		...
		if(isValid){
			...
			for(int i = 0 ; i < result[0][0].length ; ++i){
				...
				output += colSeparator + "\\multicolumn{" + colNb + "}{c|}{" + solutionHeader[i] + "}";
...
//						output += i + "\t";
						...
						if(param.cp_first_relaxation != -1 && isCpResult[i])
							output += colSeparator + al_nf.get(param.cp_first_relaxation).format(cpres.cp_first_relaxation);
						...
						if(param.cp_iteration != -1 && isCpResult[i])
							output += colSeparator + al_nf.get(param.cp_iteration).format(cpres.cp_iteration);
						
						if(param.mod_remove != -1 && isCpResult[i])
							output += colSeparator + al_nf.get(param.mod_remove).format(cpres.mod_remove);
						
						if(param.time != -1){
//							System.out.println("(n,k,i): (" + n + "," + k + "," + i + ")");
//							al_nf.get(param.time);
//							System.out.println("1");
//							System.out.println(res == null);
//							System.out.println("2");
//if(n == 0 && k == 0 && i == 3){							
//System.out.println("Param time: " + param.time);	
//System.out.println(res.time);
//}
							if(res.time != -1)
								output += colSeparator + al_nf.get(param.time).format(res.time);
							else
								output += colSeparator + "-";
						}
						...
						if(param.firstRelaxation != -1)
							output += colSeparator + al_nf.get(param.firstRelaxation).format(res.firstRelaxation);
						
						if(param.bestRelaxation != -1)
							output += colSeparator + al_nf.get(param.bestRelaxation).format(res.bestRelaxation);
						
						if(param.bestInt != -1)
							output += colSeparator + al_nf.get(param.bestInt).format(res.bestInt);
...
//TODO Comment this if?
						if(param.iterationNb != -1)
							output += colSeparator + al_nf.get(param.iterationNb).format(res.iterationNb);
						
						if(param.separationTime != -1)
							output += colSeparator + al_nf.get(param.separationTime).format(res.separationTime);
	...
	public static String get3DTables(ArrayList<ArrayList<double[][][]>> tables, ArrayList<Integer> sig, ArrayList<String> solutionHeader){
...
//				
//				/* For each K */
//				for(int k = 0 ; k < firstTable[0].length ; ++k){
//				
//					/* For each instance */
//					for(int i = 0 ; i < firstTable[0][ -1)
//						cp_header.add("CP time");
//							
//					if(param.cpCutNb != -1)
//						cp_header.add("CP cut");0].length ; ++i){
//
//						
...
//							for(int l = 0 ; l < tables.get(t).size() ; ++ l){
//								output += al_nf.get(l).format(tables.get(t).get(l)[n][k][i]) + colSeparator;
//							}
//						
//						if(i == firstTable[0][0].length-1)
//							output += endOfLine1;
//						else
//							output += endOfLine2;
//						
...
//		      System.out.println("serialize");
	...
	public static String getOutput(ArrayList<String[][]> values, String header, int nm, int nM, int km, int kM){
...
//				System.out.println(result);
	...
	/**
	 * Number of figures after the comma displayed for each parameter (if -1 the parameter is not displayed)
	 * @author zach
	 *
	 */
	...
	public class CP_ResultParamToDisplay extends ResultParamToDisplay{
		...
		public CP_ResultParamToDisplay(int gap, int bestInt, int firstRelaxation, int bestRelaxation, 
			...
			super(gap, bestInt, firstRelaxation, bestRelaxation, time, node, cutNb, iterationNb, separationTime);
		...
		if(isValid){
			...
			for(int n = 0 ; n < nValue.length ; ++n){
				...
				for(int i = 0 ; i < result[0][0].length ; ++i){
					...
					output += "\n" + colSeparator + "\\scalebox{0.3}{\\begin{tikzpicture}[baseline=(current bounding box.center)]";
					
					/* Draw a white rectangle of maximal size to ensure that all the histogram have the same size */ 
					...
					for(int k = 0 ; k < kValue.length ; ++k){
						...
						output += "\\draw [line width=14mm, color=" + color[k] + "!70] plot [ycomb] coordinates {(" + xCoordinates[k] + "," + yCoordinates[k] +  ")};";
						
//						if(cpres != null)
//								output += "\\draw [line width=14mm, color=" + color[k] + "!90] plot [ycomb] coordinates {(" + xCoordinates[k] + "," + v_cp +  ")};";	
						
						double gap = 100 * ComputeResults.improvement(res.bestRelaxation, res.bestInt);
						if(gap >= 1 && yCoordinates[k] == 3.6){
							double y = 1.8;
							output += "\\draw[white](" + xCoordinates[k] + "," + y + ")node{\\begin{Huge}\\textbf{" + Math.round(gap) + "}\\end{Huge}};";
						}
					}
	 ...
	 * @param col_first_coordinate After splitting a line according to its spaces, column in which the first coordinate is (e.g.: " test 1 3", is splitted in the four following columns numbered from 0 to 2, "", "test", "1", "3")
	 * @param col_second_coordinate
	 */
	public static void convert2DCoordinatesIntoDistanceInputFile(String input_path, String output_path, int col_first_coordinate, int col_second_coordinate){
		...
		try{
		  ...
		  while ((ligne=br.readLine())!=null){
			  ...
			  try{
				  ...
				  if(splited.length > 2)
					  coordinate.add(new Coordinate(Double.parseDouble(splited[col_first_coordinate]), Double.parseDouble(splited[col_second_coordinate])));
