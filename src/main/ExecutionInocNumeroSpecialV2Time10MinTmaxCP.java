...
public class ExecutionInocNumeroSpecialV2Time10MinTmaxCP extends Execution{
	...
	public ExecutionInocNumeroSpecialV2Time10MinTmaxCP(Cplex cplex, int nm, int nM2, int km, int kM2,
			int im, int iM2) {
	...
	public void performFormulation(int id_n, int id_k, int id_i, int gap, double gapValue, String formulation_name){
		...
		TildeParam tp2  = new TildeParam(null, cplex, c_k, true, Triangle.USE_IN_BC_ONLY, true, false, false);
		tp2.tilim = tilim;
		...
		if(resultats[id_n][id_k][gap][0][id_i] == -Double.MAX_VALUE)
		{
			...
			CP_Rep cprep;
			try {
				cprep = new CP_Rep(tp2, 500, -1,  1, 5, true, tilim);
