...
public class ExecutionTimeAcSsTildes extends Execution{
	...
	public ExecutionTimeAcSsTildes(Cplex cplex, int nm, int nM2, int km, int kM2,
			int im, int iM2) {
	...
	public void execution() throws IloException {
		...
		PartitionWithRepresentative repT = ((PartitionWithRepresentative)createPartition(new TildeParam(null, cplex, -1, false, false)));
		tildes_temps[c_n][c_k][c_i] = repT.getCplex().solve();
		bestInt[c_n][c_k][c_i] = repT.getCplex().getObjValue();
		...
		PartitionWithRepresentative repSST = ((PartitionWithRepresentative)createPartition(new RepParam(null, cplex, -1, false)));
		norm_temps[c_n][c_k][c_i] = repSST.getCplex().solve();
