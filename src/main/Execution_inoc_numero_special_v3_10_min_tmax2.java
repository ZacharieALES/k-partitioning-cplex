...
public class Execution_inoc_numero_special_v3_10_min_tmax2 extends Execution{
	...
	TildeParam tp  = new TildeParam(null, c_k, true, Triangle.USE, true, true, true);
	...
	public void performFormulation(FormulationType formulation, String formulation_name, Param param){
		...
		if(previousResult == null){
			...
			if(formulation != FormulationType.BC){
				...
				result.resolutionTime = p.solve();
				result.nodes = (int) p.getNnodes();
				result.bestRelaxation = p.getBestObjValue2();
				result.bestInteger = p.getObjValue2();
			}
			else{
